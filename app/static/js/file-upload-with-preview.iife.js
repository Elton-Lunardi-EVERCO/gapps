var FileUploadWithPreview = (function (Q) {
  ...
  const a =
    ...
    F = "files selected",
    d = "Upload",
    p = () => Math.random().toString(16).slice(2);
  class k {
    constructor(A, g = {}) {
      if (
        ((this.options = {
          ...
          text: { browse: v, chooseFile: w, label: d, selectedCount: F },
        }),
        !A)
      )
        throw new Error(
    ...
    async addImagesFromPath(A) {
      A.forEach(async (g) => {
        try {
          const E = "image/jpeg",
            B = await (await fetch(g[1], { mode: "cors" })).blob(),
            I = new File([B], g[0], { type: B.type || E });
          this.addFiles([I]);
        } catch (E) {
    ...
    addFiles(A) {
      ...
      this.options.multiple || (this.cachedFileArray = []),
        g.forEach((B) => {
          const I = new File([B], `${B.name || "fallback-name"}${c}${p()}`, {
            type: B.type,
          });
...
$(window).on("fileUploadWithPreview:imageMultiItemClicked", function (e) {
  // show image preview
  var image = new Image();
  image.src = URL.createObjectURL(e.detail.file);
  var viewer = new Viewer(image, {
    viewed: function () {
      //viewer.title.innerHTML = '<input name="title" placeholder="Enter the title" />';
    },
  });
  viewer.show();
});
