class Table {
  constructor(config) {
    ...
    (this.dataUrl = config.dataUrl), (this.ruleUrl = config.ruleUrl);
  ...
  create(title = "Default") {
    if (!this.exist()) {
      ...
      var tableDiv = $("<div>").html(
        `<table class="table table-vcenter table-bordered text-sm font-medium text-gray-500" id="table-${this.id}" style="width:100%"><thead><tr></tr></thead></table>`
      );
      var buttonDiv = $("<div>");
      ...
      if (this.refreshBtn) {
        var colButton = $("<button>")
          .attr("data-selector", `${this.id}`)
          .addClass("btn btn-square btn-ghost btn-xs mr-2 refresh-button")
          .html(
      ...
      if (this.toggleCols) {
        var colButton = $("<button>")
          .attr("data-selector", `${this.id}`)
          .addClass("btn btn-square btn-ghost btn-xs mr-2 column-button")
          .html(
      ...
      if (this.enableFilter) {
        var filterButton = $("<button>")
          .attr({ "data-selector": `${this.id}`, id: `${this.id}-filter` })
          .addClass("btn btn-square btn-ghost btn-xs filter-button")
          .html(
  ...
  createQueryModal() {
    $(document.body).append(
      `<input type="checkbox" id="${this.id}-filter-modal" class="modal-toggle" /><div class="modal"><div class="modal-box relative w-11/12 max-w-5xl"><label for="${this.id}-filter-modal" class="btn btn-sm btn-circle absolute right-2 top-2">✕</label><h3 class="text-lg font-bold">Apply Filter</h3><div class="py-4"><div id="${this.id}-builder"></div></div><div class="modal-action"><button id="${this.id}-reset" data-selector="${this.id}" class="btn btn-sm float-right mr-2 reset-filter btn-error text-white">Reset</button><button data-selector="${this.id}" class="btn btn-sm float-right mr-2 send-filter">Filter</button></div></div></div>`
    );
  }
  createColumnModal() {
    $(document.body).append(
      `<input type="checkbox" id="${this.id}-column-modal" class="modal-toggle" /><div class="modal"><div class="modal-box relative"><label for="${this.id}-column-modal" class="btn btn-sm btn-circle absolute right-2 top-2">✕</label><h3 class="text-lg font-bold">Display Columns</h3><div class="py-4"><select name="${this.id}-select" id="${this.id}-select" data-selector="${this.id}" class="dropdown-content menu p-2 shadow bg-base-100 rounded-box"><option></option></select></div><div class="modal-action"></div></div></div>`
    );
  }
  createDataModal() {
    $(document.body).append(
      `<input type="checkbox" id="${this.id}-data-modal" class="modal-toggle" /><div class="modal"><div class="modal-box w-11/12 max-w-5xl relative"><label for="${this.id}-data-modal" class="btn btn-sm btn-circle absolute right-2 top-2">✕</label><div class="card-title mb-4 justify-between"><h3 class="text-lg font-bold">Record Data</h3><div><div id="${this.id}-data-modal-link" class="mr-10"></div></div></div><div id="${this.id}-data-modal-body" class="card-body bg-gray-100 rounded-md mt-4"></div><div class="modal-action"></div></div></div>`
    );
  }
        ...
        .done(function () {
          ...
          if (config.nextLinkField) {
            $.each(data.data, function (index, record) {
              record[config.nextLinkField] = `<a href="${
                record[config.nextLinkField]
          ...
          if (data.columns.length > 0) {
            ...
            $.each(data.columns, function (k, colObj) {
              if (config.toggleCols) {
                ...
                $(selectId).append(
                  `<option value="${colObj.name}">${colObj.name}</option>`
                );
              }
            ...
            if (config.spawnModalOnClick) {
              table.on("dblclick", "tbody tr", function () {
                ...
                document.getElementById(
                  `${config.id}-data-modal`
                ).checked = true;
                var div = $("<div>").addClass("grid grid-cols-3 gap-4");
                $.each(rowData, function (key, value) {
                  var col = $("<div>")
                    .html(
                      `<label class="block text-sm font-medium text-gray-700">${key}</label><p class="text-xs text-gray-900 mt-1">${value}</p>`
                    )
                    .addClass("col-span-1");
                  div.append(col);
                });
                ...
                if (config.nextLinkField) {
                  $(`#${config.id}-data-modal-link`).html(
                    rowData[config.nextLinkField]
                  );
                }
                var rowData = table.row(this).data();
                document.getElementById(
