...
def render_questionnaire(qid):
    ...
    return render_template(
        "forms/render_form.html", questionnaire=result["extra"]["questionnaire"]
    )
...
def view_framework(name):
    framework = Framework.query.filter(
        func.lower(Framework.name) == func.lower(name)
    ).first()
    return render_template("view_framework.html", framework=framework)
...
def view_file_for_evidence(eid, name):
    ...
    if name.lower() not in result["extra"]["evidence"].get_files(basename=True):
        abort(404)
    return send_from_directory(
        directory=result["extra"]["evidence"].tenant.get_evidence_folder(),
        path=name,
        as_attachment=True,
    )
...
def view_policy(pid):
    ...
    return render_template(
        "view_policy.html", policy=policy, labels=labels, controls=controls
    )
...
@main.route(
    "/projects/<int:pid>/controls/<int:cid>/subcontrols/<int:sid>", methods=["GET"]
)
@login_required
def view_subcontrol_in_project(pid, cid, sid):
    ...
    return render_template(
        "view_subcontrol_in_project.html",
        project=subcontrol.p_control.project,
        project_subcontrol=subcontrol,
        layout=layout,
    )
...
def view_rendered_policy(pid):
    ...
    return render_template("view_rendered_policy.html", policy=policy, content=content)
