...
class Locker(LogMixin, db.Model):
    ...
    def find_by_name(name, tenant_id):
        return (
            Locker.query.filter(Locker.tenant_id == tenant_id)
            .filter(func.lower(Locker.name) == func.lower(name))
            .first()
        )
    ...
    def has_integration(self, integration_id):
        return (
            LockerAssociation.query.filter(LockerAssociation.locker_id == self.id)
            .filter(LockerAssociation.integration_id == integration_id)
            .first()
        )
...
class LockerAssociation(db.Model):
    ...
    integration_id = db.Column(
        db.Integer(), db.ForeignKey("integrations.id", ondelete="CASCADE")
    )
    date_added = db.Column(db.DateTime, default=datetime.utcnow)
    date_updated = db.Column(db.DateTime, onupdate=datetime.utcnow)
...
class Integration(LogMixin, db.Model):
    ...
    tasks = db.relationship(
        "Task", backref="integration", lazy="dynamic", cascade="all, delete-orphan"
    )
    lockers = db.relationship(
        "Locker",
        secondary="locker_association",
        lazy="dynamic",
    ...
    def add_task(self, name, cron, disabled=False):
        task = Task(
            name=name, cron=cron, disabled=disabled, queue=self.project.tenant_id
        )
...
class Task(LogMixin, db.Model):
    ...
    findings = db.relationship(
        "Finding", backref="task", lazy="dynamic", cascade="all, delete-orphan"
    )
    results = db.relationship(
        "TaskResult", backref="task", lazy="dynamic", cascade="all, delete-orphan"
    )
    ...
    def find_by_name(name, tenant_id):
        return (
            Task.query.filter(Task.tenant_id == tenant_id)
            .filter(func.lower(Task.name) == func.lower(name))
            .first()
        )
    ...
    def get_lock(self):
        ...
        """
        return (
            f"{self.integration.project.tenant_id}:{self.integration.name}:{self.name}"
        )
    ...
    def get_first_result(self, sort="id"):
        if sort == "version":
            return self.results.order_by(
                asc(cast(func.string_to_array(TaskResult.version, "."), ARRAY(Integer)))
            ).first()
    ...
    def get_latest_result(self, sort="id"):
        if sort == "version":
            return self.results.order_by(
                desc(
                    cast(func.string_to_array(TaskResult.version, "."), ARRAY(Integer))
                )
            ).first()
...
class Tenant(LogMixin, db.Model):
    ...
    user_roles = db.relationship(
        "UserRole", backref="tenant", lazy="dynamic", cascade="all, delete-orphan"
    )
    ...
    evidence = db.relationship(
        "Evidence", backref="tenant", lazy="dynamic", cascade="all, delete-orphan"
    )
    tags = db.relationship(
        "Tag", backref="tenant", lazy="dynamic", cascade="all, delete-orphan"
    )
    questionnaires = db.relationship(
        "Questionnaire", backref="tenant", lazy="dynamic", cascade="all, delete-orphan"
    )
    ...
    def find_by_name(name):
        if tenant_exists := Tenant.query.filter(
            func.lower(Tenant.name) == func.lower(name)
        ).first():
    ...
    def users(self):
        user_ids = (
            UserRole.query.filter(UserRole.tenant_id == self.id)
    ...
    def remove_user_from_questionnaires(self, user):
        for questionnaire in self.questionnaires:
            QuestionnaireGuest.query.filter(
                QuestionnaireGuest.questionnaire_id == questionnaire.id
            ).filter(QuestionnaireGuest.user_id == user.id).delete()
            db.session.commit()
        return True
        ...
        for role_name in roles:
            if not self.has_user_with_role(user, role_name):
                if role := Role.find_by_name(role_name):
                    user_role = UserRole(
                        user_id=user.id, role_id=role.id, tenant_id=self.id
                    )
    ...
    def remove_role_from_user(self, user, role_name):
        ...
        if role := Role.find_by_name(role_name):
            UserRole.query.filter(UserRole.tenant_id == self.id).filter(
                UserRole.user_id == user.id
            ).filter(UserRole.role_id == role.id).delete()
            db.session.commit()
        return True
    ...
    def get_size_of_evidence_folder(self):
        return sum(
            os.path.getsize(f)
            for f in glob.glob(f"{self.get_evidence_folder()}/*")
            if os.path.isfile(f)
        )
    ...
    def create_project(
        self, name, owner, framework=None, description=None, controls=[]
    ):
        if not description:
            description = name
        project = Project(
            name=name, description=description, owner_id=owner.id, tenant_id=self.id
        )
        if framework:
...
class Evidence(LogMixin, db.Model):
    ...
    def get_files(self, as_dict=False, basename=False):
        files = glob.glob(
            os.path.join(self.tenant.get_evidence_folder(), f"{self.id}_*")
        )
        if as_dict:
    ...
    def delete_file_by_name(self, name):
        for file in glob.glob(
            os.path.join(self.tenant.get_evidence_folder(), f"{self.id}_*_{name}")
        ):
            os.remove(file)
        return True
    ...
    def save_file(self, name, file_object):
        ...
        file_uuid = "".join(random.choices(string.ascii_uppercase + string.digits, k=7))
        full_path = os.path.join(
            self.tenant.get_evidence_folder(), f"{self.id}_{file_uuid}_{name}".lower()
        )
...
class EvidenceAssociation(db.Model):
    ...
    control_id = db.Column(
        db.Integer(), db.ForeignKey("project_subcontrols.id", ondelete="CASCADE")
    )
    evidence_id = db.Column(
        db.Integer(), db.ForeignKey("evidence.id", ondelete="CASCADE")
    )
    date_added = db.Column(db.DateTime, default=datetime.utcnow)
    date_updated = db.Column(db.DateTime, onupdate=datetime.utcnow)
...
class PolicyAssociation(LogMixin, db.Model):
    ...
    id = db.Column(db.Integer(), primary_key=True)
    policy_id = db.Column(
        db.Integer(), db.ForeignKey("policies.id", ondelete="CASCADE")
    )
    control_id = db.Column(
        db.Integer(), db.ForeignKey("controls.id", ondelete="CASCADE")
    )
    date_added = db.Column(db.DateTime, default=datetime.utcnow)
    date_updated = db.Column(db.DateTime, onupdate=datetime.utcnow)
...
class Policy(LogMixin, db.Model):
    ...
    project_policies = db.relationship(
        "ProjectPolicy", backref="policy", lazy="dynamic", cascade="all, delete"
    )
    tenant_id = db.Column(db.Integer, db.ForeignKey("tenants.id"), nullable=True)
    date_added = db.Column(db.DateTime, default=datetime.utcnow)
    date_updated = db.Column(db.DateTime, onupdate=datetime.utcnow)
...
class ProjectMember(LogMixin, db.Model):
    ...
    access_level = db.Column(
        db.String(), nullable=False, default="viewer"
    )  # manager, contributor, viewer, auditor
    date_added = db.Column(db.DateTime, default=datetime.utcnow)
    date_updated = db.Column(db.DateTime, onupdate=datetime.utcnow)
...
class Project(LogMixin, db.Model, DateMixin):
    ...
    integrations = db.relationship(
        "Integration", backref="project", lazy="dynamic", cascade="all, delete-orphan"
    )
    findings = db.relationship(
    ...
    def review_summary(self):
        ...
        data = {"total": 0}
        for record in (
            ProjectSubControl.query.with_entities(
                ProjectSubControl.review_status,
                func.count(ProjectSubControl.review_status),
            )
            .group_by(ProjectSubControl.review_status)
            .filter(ProjectSubControl.project_id == self.id)
            .all()
        ):
    ...
    def subcontrols(self, as_query=False):
        _query = ProjectSubControl.query.filter(
            ProjectSubControl.p_control.has(project_id=self.id)
        )
        if as_query:
            return _query
        return _query.all()
...
class ProjectPolicyAssociation(LogMixin, db.Model):
    ...
    id = db.Column(db.Integer(), primary_key=True)
    policy_id = db.Column(
        db.Integer(), db.ForeignKey("project_policies.id", ondelete="CASCADE")
    )
    control_id = db.Column(
        db.Integer(), db.ForeignKey("project_controls.id", ondelete="CASCADE")
    )
    date_added = db.Column(db.DateTime, default=datetime.utcnow)
    date_updated = db.Column(db.DateTime, onupdate=datetime.utcnow)
...
class ProjectPolicy(LogMixin, db.Model):
    ...
    def get_controls(self):
        controls = []
        for control in ProjectPolicyAssociation.query.filter(
...
class AuditorFeedback(LogMixin, db.Model):
    ...
    subcontrol_id = db.Column(
        db.Integer, db.ForeignKey("project_subcontrols.id"), nullable=False
    )
    date_added = db.Column(db.DateTime, default=datetime.utcnow)
    date_updated = db.Column(db.DateTime, onupdate=datetime.utcnow)
...
class ControlComment(LogMixin, db.Model):
    ...
    owner_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    control_id = db.Column(
        db.Integer, db.ForeignKey("project_controls.id"), nullable=False
    )
    date_added = db.Column(db.DateTime, default=datetime.utcnow)
    date_updated = db.Column(db.DateTime, onupdate=datetime.utcnow)
...
class ProjectSubControl(LogMixin, db.Model, SubControlMixin):
    ...
    review_status = db.Column(
        db.String(), default="not started"
    )  # ["not started","infosec action","ready for auditor","action required","complete"]
    """
    framework specific fields
    """
    # SOC2
    ...
    owner_id = db.Column(db.Integer(), db.ForeignKey("users.id"))
    subcontrol_id = db.Column(
        db.Integer, db.ForeignKey("subcontrols.id"), nullable=False
    )
...
class User(LogMixin, db.Model, UserMixin):
    ...
    def tenants(self, own=False):
        ...
        tenant_ids = (
            UserRole.query.filter(UserRole.user_id == self.id)
            .distinct(UserRole.tenant_id)
            .all()
        )
...
class Questionnaire(LogMixin, db.Model):
    ...
    guests = db.relationship(
        "QuestionnaireGuest", backref="questionnaire", lazy="dynamic"
    )
    owner_id = db.Column(db.Integer(), db.ForeignKey("users.id"), nullable=False)
    tenant_id = db.Column(db.Integer, db.ForeignKey("tenants.id"), nullable=False)
...
class Logs(db.Model):
    ...
    def get_logs(
        ...
        """
        _query = Logs.query.filter(Logs.namespace == namespace.lower()).order_by(
            Logs.id.desc()
        )
        if log_type:
