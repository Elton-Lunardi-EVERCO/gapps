...
class Authorizer:
    ...
    def return_response(self, ok, msg, code=200, **kwargs):
        data = {**{"ok": ok, "mensagem":msg, "código": code}, "extra": {**kwargs}}
    ...
    def id_to_obj(self, model_str, object):
        ...
        if not model:
            logging.error(f"objeto de tabela: {model_str} não encontrado para Autorizador!")
        try:
    ...
    def _does_tenant_exist(self, tenant):
        return self.id_to_obj("Locatário", tenant)
    ...
    def _can_user_read_tenant(self, tenant):
        if self.user.super or self.user.id == tenant.owner_id or self.user.has_any_role_for_tenant(tenant, ["admin", "editor","vizualizador"]):
            return True
        return False
    ...
    def can_user_admin_tenant(self, tenant):
        if not (tenant := self._does_tenant_exist(tenant)):
            return self.return_response(False, "locatário não encontrado", 404)
    ...
    def can_user_manage_questionnaire(self, questionnaire):
        if not (questionnaire := self.id_to_obj("Questionnaire", questionnaire)):
            return self.return_response(False, "questionário não encontrado", 404)
    ...
    def can_user_manage_tag(self, tag):
        if not (tag := self.id_to_obj("Tag", tag)):
            return self.return_response(False, "etiqueta não encontrada", 404)
    ...
    def can_user_manage_policy_label(self, label):
        if not (label := self.id_to_obj("PolicyLabels", label)):
            return self.return_response(False, "rótulo de política não encontrado", 404)
    ...
    def can_user_manage_control(self, control):
        if not (control := self.id_to_obj("Control", control)):
            return self.return_response(False, "controle não encontrado", 404)
    ...
    def can_user_manage_policy(self, policy):
        if not (policy := self.id_to_obj("Policy", policy)):
            return self.return_response(False, "política não encontrada", 404)
    ...
    def can_user_read_framework(self, framework):
        if not (framework := self.id_to_obj("Framework", framework)):
            return self.return_response(False, "framework não encontrada", 404)
    ...
    def can_user_manage_evidence(self, evidence):
        if not (evidence := self.id_to_obj("Evidence", evidence)):
            return self.return_response(False, "evidência não encontrada", 404)
    ...
    def _can_user_manage_project(self, project):
        if self._can_user_admin_tenant(project.tenant) or project.has_member_with_access(self.user, ["gerente"]):
            return True
        return False

    def _can_user_edit_project(self, project):
        if self._can_user_admin_tenant(project.tenant) or project.has_member_with_access(self.user, ["gerente", "contribuinte"]):
            return True
        return False

    def _can_user_read_project(self, project):
        if self._can_user_admin_tenant(project.tenant) or project.has_member_with_access(self.user, ["gerente", "contribuinte", "vizualizador"]):
            return True
        return False
    ...
    def can_user_manage_project(self, project):
        if not (project := self._does_project_exist(project)):
            return self.return_response(False, "projeto não encontrado", 404)
    ...
    def can_user_delete_project_comment(self, comment):
        if not (comment := self.id_to_obj("ProjectComment", comment)):
            return self.return_response(False, "comentário não encontrado", 404)
    ...
    def can_user_read_project_subcontrol(self, subcontrol):
        if not (subcontrol := self.id_to_obj("ProjectSubControl", subcontrol)):
            return self.return_response(False, "subcontrole não encontrado", 404)
    ...
    def can_user_manage_project_subcontrol_feedback(self, feedback):
        if not (feedback := self.id_to_obj("AuditorFeedback", feedback)):
            return self.return_response(False, "feedback não encontrados", 404)
    ...
    def can_user_manage_user(self, user):
        if not (user := self.id_to_obj("Usuário", user)):
            return self.return_response(False, "usuário não encontrado", 404)
    ...
    def can_user_manage_user_roles_in_tenant(self, user, tenant):
        if not (user := self.id_to_obj("Usuário", user)):
            return self.return_response(False, "usuário não encontrado", 404)
        if not (tenant := self.id_to_obj("Locatário", tenant)):
            return self.return_response(False, "locatário não encontrado", 404)
        if self._can_user_admin_tenant(tenant):
